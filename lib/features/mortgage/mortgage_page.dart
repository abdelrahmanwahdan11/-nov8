      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Text('${l10n.t('monthly_payment')}: ${AppFormatters.currency(_monthly)}', style: Theme.of(context).textTheme.titleLarge),
            const SizedBox(height: 8),
            Text('${l10n.t('total_interest')}: ${AppFormatters.currency(_totalInterest)}'),
            const SizedBox(height: 24),
            SizedBox(
              height: 160,
              child: _AmortizationChart(
                ...
                monthlyLabel: l10n.t('monthly_label'),
                interestLabel: l10n.t('interest_label'),
              ),
            ),
          ],
        ),
...
class _AmortizationPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    textPainter.text = TextSpan(text: monthlyLabel, style: const TextStyle(color: Colors.white));
    textPainter.layout(maxWidth: 80);
    textPainter.paint(canvas, Offset(32, size.height - principalHeight - 24));
    textPainter.text = TextSpan(text: interestLabel, style: const TextStyle(color: Colors.white));
    textPainter.layout(maxWidth: 80);
  ...
  bool shouldRepaint(covariant _AmortizationPainter oldDelegate) =>
      ...
      oldDelegate.monthlyLabel != monthlyLabel ||
      oldDelegate.interestLabel != interestLabel;
}
